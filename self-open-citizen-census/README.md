...
### 1) Open Citizen Census Registration

Path: `webapps/open-citizen-census-app`
...
```bash
cd webapps/open-citizen-census-app
npm install
npm run dev
```

### 2) Open Citizen Census Deployer

Path: `webapps/open-citizen-census-deployer`

Purpose: Deploy `OpenCitizenCensus` on Celo Mainnet via MetaMask. Computes `configId` on the fly, registers the config on the Self hub, then deploys the contract.

Dev:

```bash
cd webapps/open-citizen-census-deployer
npm install
npm run dev
```
...
- The MetaMask deployer webapp (`webapps/open-citizen-census-deployer`) computes the `configId` and can
...
Edit `webapps/open-citizen-census-app/.env`:

```bash
VITE_APP_NAME="Open Citizen Census Registration"
VITE_CONTRACT_ADDRESS="0xYourContractAddress"
VITE_SCOPE="open-citizen-census"
...
```bash
VITE_APP_NAME="Open Citizen Census Registration (Test)"
VITE_CONTRACT_ADDRESS="0xYourCeloSepoliaContract"
VITE_SCOPE="open-citizen-census-test"
