...
type ProofWithTotal = {
  total?: number;
  committedInputs?: Record<string, unknown>;
};
...
function decodeAlpha3(bytes: unknown): string | null {
  ...
  const decoded = new TextDecoder()
    .decode(new Uint8Array(normalized))
    .replace(/\0/g, "")
    .trim()
    .toUpperCase();

  return /^[A-Z]{3}$/.test(decoded) ? decoded : null;
}

function extractNationalityFromProofs(proofs: unknown[]): string | null {
  for (const item of proofs) {
    const proof = item as ProofWithTotal;
    ...
    const discloseInputs =
      (committedInputs["disclose_bytes"] as
        | { disclosedBytes?: unknown }
        | undefined) ??
      (committedInputs["disclose_bytes_evm"] as
        | { disclosedBytes?: unknown }
        | undefined);

    const disclosedBytes = discloseInputs?.disclosedBytes;
    if (!Array.isArray(disclosedBytes) || disclosedBytes.length < 57) {
      continue;
    }

    const idCardNationality = decodeAlpha3(disclosedBytes.slice(45, 48));
    if (idCardNationality) {
      return idCardNationality;
    }

    const passportNationality = decodeAlpha3(disclosedBytes.slice(54, 57));
...
export function useZkPassportVerification(): {
  ...
  const attemptInternalResultFallback = useCallback(
    async (trigger: string) => {
      ...
      const internalProofs = sdkInstance.topicToProofs?.[activeRequestId] as
        | unknown[]
        | undefined;

      if (!Array.isArray(internalProofs) || internalProofs.length === 0) {
        log("Fallback unavailable: no proofs yet", { trigger });
        return;
      }

      let internalQueryResult = sdkInstance.topicToResults?.[
        activeRequestId
      ] as unknown;

      if (!internalQueryResult) {
        const requestedAgeGte = Number(
          sdkInstance.topicToConfig?.[activeRequestId]?.age?.gte ?? 0,
        );
        const disclosedNationality = extractNationalityFromProofs(internalProofs);

        if (requestedAgeGte > 0 && disclosedNationality) {
          internalQueryResult = {
            age: {
              gte: {
                expected: requestedAgeGte,
                result: true,
              },
            },
            nationality: {
              disclose: {
                result: disclosedNationality,
              },
            },
          };
          log("Fallback synthesized queryResult from proofs", {
            requestedAgeGte,
    ...
    async (event: React.FormEvent<HTMLFormElement>) => {
      ...
      try {
        if (!zkpassportRef.current) {
          ...
          log("SDK instance created", { domain: window.location.hostname });
        }
        ...
        const {
          ...
          onReject,
          onError,
        } = request.gte("age", 18).disclose("nationality").done();
        ...
        onResult(async ({ verified, result: queryResult }: VerificationResult) => {
          await processVerificationResult({
            verified,
            queryResult,
            source: "sdk:onResult",
